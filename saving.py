...
def load_game():
    ...
    slot_c.close()
    print(
        "What slot do you want to load from?\n{C.r}[0] Cancel\n{C.g}[1] Slot A - {}\n{C.y}[2] Slot B - {}\n{C.b}[3] Slot C - {}\n{C.m}[4] Autosave - {}\n{C.n}"
        .format(name_a, name_b, name_c, autoname, C=C),
        end="")
    while True:
        try:
            load_slot = int(input(">"))
    ...
    try:
        ...
        print(C.r + "That save file either doesn't exist or is corrupted." +
              C.n)
        return False
    g = pickle.load(load_file)
    if g.version < G.version:
        print(
            "{C.r}ERROR! You are attempting to load this save from a older version than the current core. To recover your save without corrupting it, wait before loading your save until a safer way is implemented{C.n}"
            .format(C=C))
        return False
    else:
...
def rsm__init():
    ...
    if autoname != "Empty":
        print(
            C.m +
            "You have a game saved in the autosave slot! Use 'load' to load it."
            + C.n)
