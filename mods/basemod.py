...
def gatherer_update():
    ...
    gatherer_output = G.loot_tables["gatherer"].roll(
        round(G.jobs.all["gatherer"].count * 4 * G.colony.modifier_work))
    for thing in gatherer_output:
        G.inventory_lookup(thing).count += gatherer_output[thing]
    gatherer_output = G.loot_tables["woodcutter"].roll(
        round(G.jobs.all["woodcutter"].count * 6 * G.colony.modifier_work))
    for thing in gatherer_output:
        G.inventory_lookup(thing).count += gatherer_output[thing]
    gatherer_output = G.loot_tables["digger"].roll(
        round(G.jobs.all["digger"].count * 5 * G.colony.modifier_work))
    for thing in gatherer_output:
        G.inventory_lookup(thing).count += gatherer_output[thing]
    gatherer_output = G.loot_tables["fisher"].roll(
...
def research_update():
    research_invention = round(
        ((G.jobs.all["thinker"].count * 5) +
         (G.jobs.all["inventor"].count * 15)) * G.colony.modifier_work)
    research_math = round(
        ((G.jobs.all["mathematician"].count * 10)) * G.colony.modifier_work)
    research_science = round(
        ((G.jobs.all["scientist"].count * 10)) * G.colony.modifier_work)
    research_air = round(
        ((G.jobs.all["aviator"].count * 10)) * G.colony.modifier_work)
    ...
    if G.research.progress.invention >= G.research.needed.invention and \
       ...
       G.research.progress.aerospace >= G.research.needed.aerospace:
        ...
        print(C.custom[46] + G.research.current.name +
              " has finished researching!")
...
def population_update():
    ...
    print("{C.custom[166]}{} Elders have passed away.".format(round(
        G.colony.elder * randdeath),
                                                              C=C))
    G.colony.elder -= round(G.colony.elder * randdeath)
    print("{C.custom[190]}{} Adults have aged.".format(round(G.colony.adult *
                                                             randage),
                                                       C=C))
    G.colony.elder += round(G.colony.adult * randage)
    G.colony.adult -= round(G.colony.adult * randage)
    print("{C.custom[82]}{} Children have grown up.".format(round(
        G.colony.young * randgrow),
                                                            C=C))
    G.colony.adult += round(G.colony.young * randgrow)
    G.colony.young -= round(G.colony.young * randgrow)
    print("{C.custom[85]}{} Babies have been born.".format(round(
        G.colony.adult * randborn * G.colony.modifier_health),
                                                           C=C))
...
def research_choice():
    ...
    for technology in G.researches:
        ...
        if technology.researchable():
            print("[{}] {} - {} Invention, {} Math, {} Science, {} Aerospace".
                  format(position, technology.name, technology.invention,
                         technology.math, technology.science,
...
def force_ascend():
    if G.colony.elder + G.colony.adult + G.colony.young < 100:
        print(
            C.r +
            "Your colony didn't get big enough in order for you to be more experienced."
            + C.n)
        G.rsm__("end")
        return False
    xp_gain = 0
    ...
    print(C.custom[39] +
          "You gained {} xp points because of this colony.".format(xp_gain) +
          C.c)
    G.stats.xp += xp_gain
    levels = 0
    skillgain = 0
    ...
    if levels > 0:
        print(
            C.custom[51] +
            "You leveled up {} times! You gain {} skill points and {} talent points"
            .format(levels, skillgain, talentgain) + C.n)
    G.rsm__("end")
...
def food_consume():
    ...
    while food_saturation > 0:
        ...
        for priority in food_options:
            ...
            morale_change += (priority[food_choice].saturation *
...
def liquid_consume():
    ...
    while water_saturation > 0:
        ...
        for priority in water_options:
            ...
            morale_change += (priority[water_choice].saturation *
        ...
        if total_count == 0:
            ...
            print(C.r +
                  "Your people are thirsty. Colony health decreased by {}%".
                  format(round(2 * water_saturation / 10)) + C.n)
            break
    G.colony.morale += morale_change
    print(C.c +
...
def job_management():
    ...
    for job in G.jobs.all:
        if G.jobs.all[job] in G.jobs.unlocked:
            print("[{}] {} - {} Current Workers".format(
                position + 1, G.jobs.all[job].name, G.jobs.all[job].count))
            position += 1
    ...
    if action == "a":
        print(
            C.c +
            "You have {}/{} workers currently assigned.\nHow many do you want to assign (0 = cancel)?"
            .format(G.colony.workers[0], G.colony.workers[1]) + C.n)
        while True:
            try:
                amount = int(input(">"))
            except:
        ...
        print(
            C.m +
            "You have {} workers currently assigned to that job.\nHow many do you want to unassign (0 = cancel)?"
            .format(selection.count) + C.n)
        while True:
            try:
                amount = int(input(">"))
            except:
            ...
            if amount == 0:
                ...
                print(C.r +
                      "You don't have that many workers assigned to that job" +
                      C.n)
                continue
            else:
                break
...
def rsm__tick():
    ...
    if G.eotw <= 1:
        ...
        print(C.r +
              "You are forced to ascend to avoid the terrors of the end." +
              C.n)
        force_ascend()
    elif G.eotw <= 5:
        print(
            "{C.custom[203]}The earth rumbles... The end of the world is coming...{C.n}"
            .format(C=C))
...
def change_colony_name():
    print(C.m +
          "After much discussion, you decide to change your colony name to " +
          C.n)
    name = input(">")
    G.colony_name = name
...
def rsm__init():
    ...
    #Initialize jobs
    G.jobs.all["gatherer"] = G.Job(C.custom[46] + "Gatherer" + C.n, "gatherer",
                                   0)
    G.jobs.all["woodcutter"] = G.Job(C.custom[46] + "Woodcutter" + C.n,
                                     "woodcutter", 0)
    ...
    G.jobs.all["inventor"] = G.Job(C.custom[87] + "Inventor" + C.n, "inventor",
                                   0)
    G.jobs.all["scientist"] = G.Job(C.custom[87] + "Scientist" + C.n,
                                    "scientist", 0)
    G.jobs.all["mathematician"] = G.Job(C.custom[87] + "Mathematician" + C.n,
                                        "mathematician", 0)
    G.jobs.all["aviator"] = G.Job(C.custom[87] + "Aviation Engineer" + C.n,
                                  "aviator", 0)
    G.jobs.all["crafter"] = G.Job(C.custom[226] + "Crafter" + C.n, "crafter",
                                  0)
    G.jobs.all["toolmaker"] = G.Job(C.custom[226] + "Toolmaker" + C.n,
                                    "toolmaker", 0)
    G.jobs.all["carpenter"] = G.Job(C.custom[226] + "Carpenter" + C.n,
                                    "carpenter", 0)
    G.jobs.all["blacksmith"] = G.Job(C.custom[226] + "Blacksmith" + C.n,
                                     "blacksmith", 0)
    G.jobs.all["firemaker"] = G.Job(C.custom[196] + "Firemaker" + C.n,
                                    "firemaker", 0)
    G.jobs.all["smoker"] = G.Job(C.custom[196] + "Smoker" + C.n, "smoker", 0)

    #Create Resource CateGories
    G.inventory["food"] = G.Category(C.custom[40] + "Food" + C.n, [], "food")
    G.inventory["liquid"] = G.Category(C.custom[49] + "Liquids" + C.n, [],
                                       "liquid")
    G.inventory["primitive"] = G.Category(
        C.custom[130] + "Primitive Resources" + C.n, [], "primitive")
    G.inventory["equipment"] = G.Category(C.custom[249] + "Equipment" + C.n,
                                          [], "equipment")
    ...
    G.inventory["primitive"].items.append(
        G.Resource("Sticks", 0, "primitive", "sticks"))
    G.inventory["primitive"].items.append(
        G.Resource("Rocks", 0, "primitive", "rocks"))
    G.inventory["primitive"].items.append(
        G.Resource("Leaves", 0, "primitive", "leaves"))
    G.inventory["primitive"].items.append(
        G.Resource("Logs", 0, "primitive", "logs"))
    G.inventory["primitive"].items.append(
        G.Resource("Clay", 0, "primitive", "clay"))
    G.inventory["primitive"].items.append(
        G.Resource("Mud", 0, "primitive", "mud"))

    #Define Equipment
    G.inventory["equipment"].items.append(
        G.Tool("Sharp Rocks", 0, 0, "equipment", "sharp-rocks"))
    G.inventory["equipment"].items.append(
        G.Tool("Primitive Tools", 0, 0, "equipment", "primitive-tools"))
    G.inventory["equipment"].items.append(
        G.Tool(C.custom[196] + "Campfire" + C.n, 0, 0, "equipment", "fire"))
    ...
    G.researches.append(
        G.Technology("Theory of Mathematics", "Arithmetic and more", "ToM", 50,
                     0, 0, 0, "math"))
    G.researches.append(
        G.Technology("Advanced Invention", "The magic of inventors",
                     "adv-invention", 150, 75, 0, 0, "adv_invention", ["ToM"]))
    G.researches.append(
        G.Technology("Theory of Science", "Why???", "ToS", 350, 100, 0, 0,
                     "science", ["adv-invention"]))
    G.researches.append(
        G.Technology("Theory of Aviation", "CAN HUMANS FLY?!?!?", "ToA", 650,
                     500, 250, 0, "aviation", ["ToS"]))
    G.researches.append(
        G.Technology("Theory of Outer Space", "Beyond the visible sky", "ToOS",
                     1250, 1000, 750, 750, "outer_space", ["ToA"]))

    #Processing Advancement
    G.researches.append(
        G.Technology("Production", "Raw stuff + effort = new stuff",
                     "production", 20, 0, 0, 0, "production"))
    G.researches.append(
        G.Technology("Crafting", "Producing stuff out of other stuff",
                     "crafting", 35, 0, 0, 0, "crafting", ["production"]))
    G.researches.append(
        G.Technology("Rock Striking", "rock + rock = sharp rock", "striking",
                     50, 0, 0, 0, "striking", ["crafting"]))
    G.researches.append(
        G.Technology("Tool Making", "Need the right tools for the job?",
                     "tools", 100, 0, 0, 0, "tools", ["striking"]))
    G.researches.append(
        G.Technology("Mass Crafting", "Producing multiple things at once",
                     "mass-crafting", 100, 25, 0, 0, "mass_crafting",
                     ["ToM", "crafting"]))

    #Survival Advancement
    G.researches.append(
        G.Technology("Survivalibility",
                     "Preveting your colony from going extinct", "survival",
                     25, 0, 0, 0, "survival"))
    G.researches.append(
        G.Technology("Fishing", "Catching fish out of the water", "fishing",
                     50, 0, 0, 0, "fishing", ["survival"]))
    G.researches.append(
        G.Technology("Woodcutting", "TIMBER", "woodcutting", 50, 0, 0, 0,
                     "woodcutting", ["survival"]))
    G.researches.append(
        G.Technology("Digging", "Out of the ground", "digging", 75, 0, 0, 0,
                     "digging", ["survival"]))
    G.researches.append(
        G.Technology("Firemaking", "Don't burn yourself", "firemaking", 75, 0,
                     0, 0, "firemaking", ["survival"]))
    G.researches.append(
        G.Technology("Cooking", "Don't burn the food", "cooking", 150, 0, 0, 0,
                     "cooking", ["firemaking", "fishing"]))

    #Colony Efficiency

    #Storage Advancement
    G.researches.append(
        G.Technology("Stockpiling",
                     "Putting all of your food in one central place",
                     "storing", 50, 0, 0, 0, "storing"))
    G.researches.append(
        G.Technology("Storage Efficiency I",
                     "Using a bit of math to store more efficiently", "seI",
                     125, 50, 0, 0, "seI", ["ToM", "storing"]))
    G.researches.append(
        G.Technology("Storage Efficiency II", "More math = more storage",
                     "seII", 250, 150, 0, 0, "seII", ["seI"]))
    G.researches.append(
        G.Technology("Storage Efficiency III", "NEED MOAR STORAGE", "seIII",
                     500, 300, 0, 0, "seIII", ["seII"]))
    G.researches.append(
        G.Technology("Storage Efficiency IV", "MOAAAAAAAAAR", "seIV", 1000,
                     600, 0, 0, "seIV", ["seIII"]))
    G.researches.append(
        G.Technology("Storage Efficiency V", "MAXIMUM STORAGE", "seV", 2000,
                     1250, 0, 0, "seV", ["seIV"]))
    G.researches.append(
        G.Technology("Material Duplication",
                     "bending space and time to duplicate items in storage",
                     "duplicate", 5000, 2500, 2500, 2500, "duplicate",
                     ["seV", "ToOS"]))
    G.researches.append(
        G.Technology(
            "Duplication Efficiency I",
            "improved duplication systems allows more duplication every year",
            "deI", 10000, 5000, 5000, 5000, "deI", ["duplicate"]))
